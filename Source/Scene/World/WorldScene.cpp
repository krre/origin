...
void WorldScene::init() {
    ...
    ShaderResource* shaderResource = worldShaderProgram.shaderResources[ShaderProgram::Type::VERTEX];
    graphicsPipeline.addShaderCode(VK_SHADER_STAGE_VERTEX_BIT, shaderResource->getSize() * sizeof(uint32_t), shaderResource->getData());

    shaderResource = worldShaderProgram.shaderResources[ShaderProgram::Type::FRAGMENT];
