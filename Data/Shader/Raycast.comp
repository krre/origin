...
Ray constructRay(in int index) {
    ...
    ray.direction = normalize(startCornerPos + stepW * gl_GlobalInvocationID.x + stepH * gl_GlobalInvocationID.y);
    return ray;
}
...
void main() {
    ...
    if (gl_GlobalInvocationID.y == (pickPixel.y + 0.5) && gl_GlobalInvocationID.x == (pickPixel.x + 0.5)) { // For OctreeFarm pick node
    ...
    imageStore(surface, coords, color);
}
