...
#ifdef OS_WIN
    surface = std::make_unique<Vulkan::Win32Surface>(VulkanCore::get()->getInstance(), VulkanCore::get()->getGraphicsDevice()->getPhysicalDevice()->getHandle(), GetModuleHandle(nullptr), wminfo.info.win.window);
#elif OS_LINUX
    surface = std::make_unique<Vulkan::XcbSurface>(VulkanCore::get()->getInstance(), VulkanCore::get()->getGraphicsDevice()->getPhysicalDevice()->getHandle(), XGetXCBConnection(wminfo.info.x11.display), wminfo.info.x11.window);
#endif
