...
RenderLayerDebug::RenderLayerDebug(Vulkan::Device* device, Object* parent) : RenderLayer(device, parent) {
...
RenderLayerDebug::~RenderLayerDebug() {

}

void RenderLayerDebug::write(Vulkan::CommandBuffer* commandBuffer, Vulkan::Framebuffer* framebuffer) {
    if (vertextCount) {
...
void RenderLayerDebug::resizeVertexBuffer(uint32_t size) {
    vertexBuffer = std::make_unique<Vulkan::GpuBuffer>(getDevice(), VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, size);
}

void RenderLayerDebug::setVertexCount(uint32_t vertextCount) {
...
void RenderLayerDebug::updateMvp(const glm::mat4& mvp) {
